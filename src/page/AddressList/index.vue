<template>
    <div class="AddressList" style="min-height: 100vh;">
        <van-nav-bar safe-area-inset-top fixed :title="$t('地址')" left-arrow @click-left="onClickLeft" />
        <div style="width: 100%;" class="h46"></div>
        <div class="van-address-list">
            <div v-if="list.length" role="radiogroup" class="van-radio-group">
                <div v-for="(item, index) in list" :key="index" @click="xuanzhong(item, index)">
                    <van-swipe-cell>
                        <div class="address-item">
                            ...
                            <div class="content">
                                <div class="info">
                                    ...
                                    <p>{{ item.tel }}</p>
                                    <div v-if="item.isDefault">{{ $t('默认') }}</div>
                                </div>
                                <p>{{ item.address }}</p>
                            </div>
                        </div>
                        <template #right>
                            <div class="btn edit" @click.stop="xiugai(item)">{{ $t('编辑') }}</div>
                            <div class="btn del" @click.stop="shanchu(item)">{{ $t('删除') }}</div>
                        </template>
                    </van-swipe-cell>
                </div>
                <!-- <div class="van-address-item" v-for="(item,index) in list" :key="index" @click="xuanzhong(item,index)"> -->
                <!-- <div class="van-cell van-cell--borderless" style="justify-content: center;align-items: center;">
            <div class="van-cell__value van-cell__value--alone van-address-item__value" style="flex: 1;">
              <div class="van-address-item__name">{{ item.name }} {{ item.tel }}<span v-if="item.isDefault"
                  class="van-tag van-tag--round van-tag--danger van-address-item__tag">{{ $t('默认') }}</span></div>
              <div class="van-address-item__address">{{ item.address }}</div>
            </div>
            <div class="cz">
              <van-icon name="edit" size="20" @click="xiugai(item)" style="margin-right: 12px;" />
            </div>
            <div class="" @click="shanchu(item)">
              <van-icon name="delete-o" size="20"  />
            </div>
          </div> -->
                <!-- </div> -->
            </div>
            <van-empty v-if="!pageLoading && !list.length" :description="$t('暂无数据')" />
            <div class="van-address-list__bottom">
                <button @click="onAdd"
                    ...
                    <div class="van-button__content"><span class="van-button__text">{{ $t("新增地址") }}</span></div>
                </button>
            </div>
        </div>
    </div>
...
<script>
import { NavBar, Icon, Dialog, Button, SwipeCell, Empty, Toast } from 'vant';
...
export default {
    ...
    methods: {
        ...
        xiugai(e) {
            this.$router.push({ path: '/AddressDetails?id=' + e.id2 })
        },
        shanchu(e) {
            // console.log(e)
            Dialog.confirm({
        ...
        shanchu_post(e) {
            console.log(e)
            dizhi_shanchu_post({ id: e.id2 }).then(res => {
                console.log(res)
                this.list = []
                this.dizhiliebiao()
            })
        ...
        onAdd() {
            // Toast('新增地址');
            this.$router.push({ path: '/AddressDetails' })
        },
